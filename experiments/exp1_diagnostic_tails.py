...
def run_diagnostic():
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        for res in tqdm(executor.map(process_func, episodes), total=len(episodes)):
            if "Error" not in res: 
                ...
                errors.append(res)
                # 使用 tqdm.write 防止吞并日志
                tqdm.write(f"❌ [Error in {res.get('Episode', 'Unknown')}]: {res['Error']}")
    ...
    # ==========================================
    # 🏆 定量病理分析 (Quantitative Pathology Analysis)
    # ==========================================
    print("\n" + "="*60)
    ...
    print(f"🌍 全局原始成功率 (All {len(df)} Episodes): {raw_success_rate:.2f}%")
    ...
    if len(df_pathology_A) > 0:
        ...
        print(f"🚨 [病理 A] 无激励区 (Z < 1e-6): 共有 {len(df_pathology_A)} 个样本，崩溃率为 {fail_rate_A:.2f}%")
        
    if len(df_pathology_B) > 0:
        fail_rate_B = np.mean(df_pathology_B['Correlation'] < success_threshold) * 100
        print(f"🚨 [病理 B] 极端干扰区 (干扰 > 1.0): 共有 {len(df_pathology_B)} 个样本，崩溃率为 {fail_rate_B:.2f}%")
